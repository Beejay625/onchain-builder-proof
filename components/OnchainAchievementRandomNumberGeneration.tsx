...
interface OnchainAchievementRandomNumberGenerationProps {
  achievementId: bigint
}

export default function OnchainAchievementRandomNumberGeneration({ achievementId }: OnchainAchievementRandomNumberGenerationProps) {
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">RandomNumberGeneration</h3>
      <p className="text-sm text-gray-600 mb-4">Track RandomNumberGeneration operations and verifications.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !contractAddress.startsWith('0x')} className="w-full px-4 py-3 bg-rose-600 text-white rounded-lg font-semibold hover:bg-rose-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record RandomNumberGeneration'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-rose-700 bg-rose-50 border border-rose-200 rounded-lg p-3">✓ RandomNumberGeneration recorded onchain.</div>}
    </section>
  )
}
