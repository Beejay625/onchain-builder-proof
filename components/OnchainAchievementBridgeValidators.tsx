...
interface OnchainAchievementBridgeValidatorsProps {
  achievementId: bigint
}

export default function OnchainAchievementBridgeValidators({ achievementId }: OnchainAchievementBridgeValidatorsProps) {
  ...
  const record = () => {
    ...
    if (!data.trim()) return
    const payload = `BridgeValidators|chainId:${chainId}|data:${data}|tx:${txHash}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">BridgeValidators</h3>
      <p className="text-sm text-gray-600 mb-4">Track BridgeValidators operations in cross-chain protocols.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !chainId.trim()} className="w-full px-4 py-3 bg-violet-600 text-white rounded-lg font-semibold hover:bg-violet-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record BridgeValidators'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-violet-700 bg-violet-50 border border-violet-200 rounded-lg p-3">✓ BridgeValidators recorded onchain.</div>}
    </section>
  )
}
