...
interface OnchainAchievementMultiSignatureApprovalsProps {
  achievementId: bigint
}

export default function OnchainAchievementMultiSignatureApprovals({ achievementId }: OnchainAchievementMultiSignatureApprovalsProps) {
  ...
  const record = () => {
    ...
    const payload = `MultiSignatureApprovals|contract:${contractAddress}|role:${role}|delay:${delay}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">MultiSignatureApprovals</h3>
      <p className="text-sm text-gray-600 mb-4">Track MultiSignatureApprovals operations and access configurations.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !contractAddress.startsWith('0x')} className="w-full px-4 py-3 bg-red-600 text-white rounded-lg font-semibold hover:bg-red-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record MultiSignatureApprovals'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg p-3">✓ MultiSignatureApprovals recorded onchain.</div>}
    </section>
  )
}
