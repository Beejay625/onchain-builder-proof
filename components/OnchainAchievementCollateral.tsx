...
interface OnchainAchievementCollateralProps {
  achievementId: bigint
}

export default function OnchainAchievementCollateral({ achievementId }: OnchainAchievementCollateralProps) {
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">Collateral</h3>
      <p className="text-sm text-gray-600 mb-4">Track Collateral operations in DeFi protocols.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !tokenAddress.startsWith('0x')} className="w-full px-4 py-3 bg-amber-600 text-white rounded-lg font-semibold hover:bg-amber-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record Collateral'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-3">✓ Collateral operation recorded.</div>}
    </section>
  )
}
