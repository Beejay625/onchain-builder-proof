...
interface OnchainAchievementReownWalletConnectProps {
  achievementId: bigint
}

export default function OnchainAchievementReownWalletConnect({ achievementId }: OnchainAchievementReownWalletConnectProps) {
  ...
  const record = () => {
    ...
    const payload = `ReownWalletConnect|contract:${contractAddress}|tx:${txHash}|data:${data}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">ReownWalletConnect</h3>
      <p className="text-sm text-gray-600 mb-4">Track ReownWalletConnect operations and state changes.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !contractAddress.startsWith('0x')} className="w-full px-4 py-3 bg-emerald-600 text-white rounded-lg font-semibold hover:bg-emerald-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record ReownWalletConnect'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg p-3">✓ ReownWalletConnect recorded onchain.</div>}
    </section>
  )
}
