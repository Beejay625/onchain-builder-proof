'use client'

import { useAccount, useWriteContract, useWaitForTransactionReceipt } from 'wagmi'
import { BUILDER_PROOF_CONTRACT } from '@/lib/constants'
import { BuilderProofABI } from '@/abi/BuilderProof'
import AppCard from '@/components/common/AppCard'
import { useState } from 'react'
import { isFeatureEnabled } from '@/lib/featureFlags'

export default function BuilderCertificates() {
  const { address, isConnected } = useAccount()
  const [certificateName, setCertificateName] = useState('')
  const [issuingOrganization, setIssuingOrganization] = useState('')
  const [certificateUrl, setCertificateUrl] = useState('')
  const [completionDate, setCompletionDate] = useState('')
  const { writeContract, data: hash, isPending } = useWriteContract()
  const { isLoading: isConfirming, isSuccess: isConfirmed } = useWaitForTransactionReceipt({ hash })

  if (!isFeatureEnabled('onchainBuilderCertificates')) {
    return null
  }

  const handleMintCertificate = async () => {
    if (!isConnected || !address || !certificateName.trim() || !issuingOrganization.trim()) return

    try {
      const content = `Builder Certificate Minted\nCertificate: ${certificateName}\nIssued By: ${issuingOrganization}${certificateUrl ? `\nURL: ${certificateUrl}` : ''}${completionDate ? `\nCompleted: ${completionDate}` : ''}`
      writeContract({
        address: BUILDER_PROOF_CONTRACT as `0x${string}`,
        abi: BuilderProofABI,
        functionName: 'createPost',
        args: [content],
      })
    } catch (error) {
      console.error('Certificate minting failed:', error)
    }
  }

  return (
    <AppCard title="🎓 Builder Certificates" subtitle="Mint certificates for completed courses" accent="indigo">
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Certificate Name</label>
          <input
            type="text"
            value={certificateName}
            onChange={(e) => setCertificateName(e.target.value)}
            placeholder="e.g., Solidity Developer Certification"
            className="w-full rounded-lg border border-gray-300 p-2 text-sm"
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Issuing Organization</label>
          <input
            type="text"
            value={issuingOrganization}
            onChange={(e) => setIssuingOrganization(e.target.value)}
            placeholder="e.g., Ethereum Foundation"
            className="w-full rounded-lg border border-gray-300 p-2 text-sm"
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Certificate URL (optional)</label>
          <input
            type="text"
            value={certificateUrl}
            onChange={(e) => setCertificateUrl(e.target.value)}
            placeholder="https://..."
            className="w-full rounded-lg border border-gray-300 p-2 text-sm"
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Completion Date (optional)</label>
          <input
            type="date"
            value={completionDate}
            onChange={(e) => setCompletionDate(e.target.value)}
            className="w-full rounded-lg border border-gray-300 p-2 text-sm"
          />
        </div>
        <button
          onClick={handleMintCertificate}
          disabled={isPending || isConfirming || !isConnected || !certificateName.trim() || !issuingOrganization.trim()}
          className="w-full rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700 disabled:bg-gray-400"
        >
          {isPending || isConfirming ? 'Minting...' : 'Mint Certificate'}
        </button>
        {isConfirmed && (
          <div className="rounded-lg bg-green-50 border border-green-200 p-3 text-sm text-green-800">
            ✅ Certificate minted onchain
          </div>
        )}
      </div>
    </AppCard>
  )
}

