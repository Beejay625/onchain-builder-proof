'use client'

import { useAccount, useWriteContract, useWaitForTransactionReceipt } from 'wagmi'
import { BUILDER_PROOF_CONTRACT } from '@/lib/constants'
import { BuilderProofABI } from '@/abi/BuilderProof'
import AppCard from '@/components/common/AppCard'
import { useState } from 'react'
import { isFeatureEnabled } from '@/lib/featureFlags'

interface AchievementRiskAssessmentProps {
  achievementId: bigint
}

export default function AchievementRiskAssessment({ achievementId }: AchievementRiskAssessmentProps) {
  const { address, isConnected } = useAccount()
  const [riskLevel, setRiskLevel] = useState<'low' | 'medium' | 'high' | 'critical'>('low')
  const [riskDescription, setRiskDescription] = useState('')
  const { writeContract, data: hash, isPending } = useWriteContract()
  const { isLoading: isConfirming, isSuccess: isConfirmed } = useWaitForTransactionReceipt({ hash })

  if (!isFeatureEnabled('achievementRiskAssessment')) {
    return null
  }

  const handleAssessRisk = async () => {
    if (!isConnected || !address || !riskDescription.trim()) return

    try {
      const content = `Risk Assessment\nAchievement: #${achievementId.toString()}\nLevel: ${riskLevel}\nDescription: ${riskDescription}`
      writeContract({
        address: BUILDER_PROOF_CONTRACT as `0x${string}`,
        abi: BuilderProofABI,
        functionName: 'createPost',
        args: [content],
      })
    } catch (error) {
      console.error('Risk assessment failed:', error)
    }
  }

  return (
    <AppCard title="⚠️ Achievement Risk Assessment" subtitle="Assess and record risks for achievements" accent="red">
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Risk Level</label>
          <select
            value={riskLevel}
            onChange={(e) => setRiskLevel(e.target.value as 'low' | 'medium' | 'high' | 'critical')}
            className="w-full rounded-lg border border-gray-300 p-2 text-sm"
          >
            <option value="low">Low</option>
            <option value="medium">Medium</option>
            <option value="high">High</option>
            <option value="critical">Critical</option>
          </select>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Risk Description</label>
          <textarea
            value={riskDescription}
            onChange={(e) => setRiskDescription(e.target.value)}
            placeholder="Describe the risk..."
            rows={3}
            className="w-full rounded-lg border border-gray-300 p-2 text-sm"
          />
        </div>
        <button
          onClick={handleAssessRisk}
          disabled={isPending || isConfirming || !isConnected || !riskDescription.trim()}
          className="w-full rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700 disabled:bg-gray-400"
        >
          {isPending || isConfirming ? 'Assessing...' : 'Assess Risk'}
        </button>
        {isConfirmed && (
          <div className="rounded-lg bg-green-50 border border-green-200 p-3 text-sm text-green-800">
            ✅ Risk assessment recorded onchain
          </div>
        )}
      </div>
    </AppCard>
  )
}

