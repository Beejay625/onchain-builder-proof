'use client'

import { useAccount, useWriteContract, useWaitForTransactionReceipt } from 'wagmi'
import { BUILDER_PROOF_CONTRACT } from '@/lib/constants'
import { BuilderProofABI } from '@/abi/BuilderProof'
import AppCard from '@/components/common/AppCard'
import { useState } from 'react'
import { isFeatureEnabled } from '@/lib/featureFlags'

interface AchievementLimitOrdersProps {
  achievementId: bigint
}

export default function AchievementLimitOrders({ achievementId }: AchievementLimitOrdersProps) {
  const { address, isConnected } = useAccount()
  const [tokenPair, setTokenPair] = useState('ETH/USDC')
  const [limitPrice, setLimitPrice] = useState('2000')
  const [orderSide, setOrderSide] = useState<'buy' | 'sell'>('buy')
  const { writeContract, data: hash, isPending } = useWriteContract()
  const { isLoading: isConfirming, isSuccess: isConfirmed } = useWaitForTransactionReceipt({ hash })

  if (!isFeatureEnabled('achievementLimitOrders')) {
    return null
  }

  const handleRecordOrder = async () => {
    if (!isConnected || !address || !limitPrice.trim()) return

    try {
      const content = `Limit Order\nAchievement: #${achievementId.toString()}\nPair: ${tokenPair}\nSide: ${orderSide}\nPrice: ${limitPrice}`
      writeContract({
        address: BUILDER_PROOF_CONTRACT as `0x${string}`,
        abi: BuilderProofABI,
        functionName: 'createPost',
        args: [content],
      })
    } catch (error) {
      console.error('Limit order log failed:', error)
    }
  }

  return (
    <AppCard title="📈 Limit Orders" subtitle="Log onchain limit order placements" accent="blue">
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Token Pair</label>
          <input
            type="text"
            value={tokenPair}
            onChange={(e) => setTokenPair(e.target.value)}
            className="w-full rounded-lg border border-gray-300 p-2 text-sm"
          />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Limit Price</label>
            <input
              type="text"
              value={limitPrice}
              onChange={(e) => setLimitPrice(e.target.value)}
              className="w-full rounded-lg border border-gray-300 p-2 text-sm"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Side</label>
            <select
              value={orderSide}
              onChange={(e) => setOrderSide(e.target.value as 'buy' | 'sell')}
              className="w-full rounded-lg border border-gray-300 p-2 text-sm"
            >
              <option value="buy">Buy</option>
              <option value="sell">Sell</option>
            </select>
          </div>
        </div>
        <button
          onClick={handleRecordOrder}
          disabled={isPending || isConfirming || !isConnected || !limitPrice.trim()}
          className="w-full rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:bg-gray-400"
        >
          {isPending || isConfirming ? 'Recording...' : 'Record Limit Order'}
        </button>
        {isConfirmed && (
          <div className="rounded-lg bg-green-50 border border-green-200 p-3 text-sm text-green-800">
            ✅ Limit order recorded
          </div>
        )}
      </div>
    </AppCard>
  )
}

