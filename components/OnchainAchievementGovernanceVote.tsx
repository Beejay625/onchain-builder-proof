...
interface OnchainAchievementGovernanceVoteProps {
  achievementId: bigint
}

export default function OnchainAchievementGovernanceVote({ achievementId }: OnchainAchievementGovernanceVoteProps) {
  ...
  const record = () => {
    ...
    if (!data.trim()) return
    const payload = `GovernanceVote|id:${proposalId}|data:${data}|tx:${txHash}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">GovernanceVote</h3>
      <p className="text-sm text-gray-600 mb-4">Track GovernanceVote operations in DAO governance.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !proposalId.trim()} className="w-full px-4 py-3 bg-slate-600 text-white rounded-lg font-semibold hover:bg-slate-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record GovernanceVote'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-slate-700 bg-slate-50 border border-slate-200 rounded-lg p-3">✓ GovernanceVote recorded onchain.</div>}
    </section>
  )
}
