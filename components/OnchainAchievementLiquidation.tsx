...
interface OnchainAchievementLiquidationProps {
  achievementId: bigint
}

export default function OnchainAchievementLiquidation({ achievementId }: OnchainAchievementLiquidationProps) {
  ...
  const record = () => {
    ...
    const payload = `Liquidation|token:${tokenAddress}|data:${data}|tx:${txHash}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">Liquidation</h3>
      <p className="text-sm text-gray-600 mb-4">Track Liquidation operations in DeFi protocols.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !tokenAddress.startsWith('0x')} className="w-full px-4 py-3 bg-amber-600 text-white rounded-lg font-semibold hover:bg-amber-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record Liquidation'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-3">✓ Liquidation operation recorded.</div>}
    </section>
  )
}
