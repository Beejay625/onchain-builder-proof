...
interface OnchainAchievementERC721Props {
  achievementId: bigint
}

export default function OnchainAchievementERC721({ achievementId }: OnchainAchievementERC721Props) {
  ...
  const record = () => {
    ...
    const payload = `ERC721|contract:${contractAddress}|impl:${implementation}|tx:${txHash}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">ERC721</h3>
      <p className="text-sm text-gray-600 mb-4">Track ERC721 token standard implementation.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !contractAddress.startsWith('0x')} className="w-full px-4 py-3 bg-pink-600 text-white rounded-lg font-semibold hover:bg-pink-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record ERC721'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-pink-700 bg-pink-50 border border-pink-200 rounded-lg p-3">✓ ERC721 implementation recorded.</div>}
    </section>
  )
}
