...
interface OnchainAchievementLightningNetworkProps {
  achievementId: bigint
}

export default function OnchainAchievementLightningNetwork({ achievementId }: OnchainAchievementLightningNetworkProps) {
  ...
  const record = () => {
    ...
    const payload = `LightningNetwork|channel:${channelId}|p1:${participant1}|p2:${participant2}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">LightningNetwork</h3>
      <p className="text-sm text-gray-600 mb-4">Track LightningNetwork off-chain payment channels.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !channelId.startsWith('0x')} className="w-full px-4 py-3 bg-emerald-600 text-white rounded-lg font-semibold hover:bg-emerald-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record LightningNetwork'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg p-3">✓ LightningNetwork channel recorded.</div>}
    </section>
  )
}
