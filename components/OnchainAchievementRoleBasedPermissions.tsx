...
interface OnchainAchievementRoleBasedPermissionsProps {
  achievementId: bigint
}

export default function OnchainAchievementRoleBasedPermissions({ achievementId }: OnchainAchievementRoleBasedPermissionsProps) {
  ...
  const record = () => {
    ...
    const payload = `RoleBasedPermissions|contract:${contractAddress}|role:${role}|delay:${delay}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">RoleBasedPermissions</h3>
      <p className="text-sm text-gray-600 mb-4">Track RoleBasedPermissions operations and access configurations.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !contractAddress.startsWith('0x')} className="w-full px-4 py-3 bg-red-600 text-white rounded-lg font-semibold hover:bg-red-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record RoleBasedPermissions'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg p-3">✓ RoleBasedPermissions recorded onchain.</div>}
    </section>
  )
}
