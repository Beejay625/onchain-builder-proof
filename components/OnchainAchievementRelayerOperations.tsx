...
interface OnchainAchievementRelayerOperationsProps {
  achievementId: bigint
}

export default function OnchainAchievementRelayerOperations({ achievementId }: OnchainAchievementRelayerOperationsProps) {
  ...
  const record = () => {
    ...
    const payload = `RelayerOperations|chainId:${chainId}|data:${data}|tx:${txHash}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">RelayerOperations</h3>
      <p className="text-sm text-gray-600 mb-4">Track RelayerOperations operations in cross-chain protocols.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !chainId.trim()} className="w-full px-4 py-3 bg-violet-600 text-white rounded-lg font-semibold hover:bg-violet-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record RelayerOperations'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-violet-700 bg-violet-50 border border-violet-200 rounded-lg p-3">✓ RelayerOperations recorded onchain.</div>}
    </section>
  )
}
