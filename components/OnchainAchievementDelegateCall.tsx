...
interface OnchainAchievementDelegateCallProps {
  achievementId: bigint
}

export default function OnchainAchievementDelegateCall({ achievementId }: OnchainAchievementDelegateCallProps) {
  ...
  const record = () => {
    ...
    const payload = `DelegateCall|data:${data}|tx:${txHash}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">DelegateCall</h3>
      <p className="text-sm text-gray-600 mb-4">Track DelegateCall operations onchain.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming} className="w-full px-4 py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record DelegateCall'}
      </button>
