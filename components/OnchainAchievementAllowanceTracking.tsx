...
interface OnchainAchievementAllowanceTrackingProps {
  achievementId: bigint
}

export default function OnchainAchievementAllowanceTracking({ achievementId }: OnchainAchievementAllowanceTrackingProps) {
  ...
  const record = () => {
    ...
    const payload = `AllowanceTracking|token:${tokenAddress}|account:${account}|value:${value}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">AllowanceTracking</h3>
      <p className="text-sm text-gray-600 mb-4">Track AllowanceTracking operations and token states.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !tokenAddress.startsWith('0x')} className="w-full px-4 py-3 bg-slate-600 text-white rounded-lg font-semibold hover:bg-slate-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record AllowanceTracking'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-slate-700 bg-slate-50 border border-slate-200 rounded-lg p-3">✓ AllowanceTracking recorded onchain.</div>}
    </section>
  )
}
