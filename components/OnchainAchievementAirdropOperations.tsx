...
interface OnchainAchievementAirdropOperationsProps {
  achievementId: bigint
}

export default function OnchainAchievementAirdropOperations({ achievementId }: OnchainAchievementAirdropOperationsProps) {
  ...
  const record = () => {
    ...
    const payload = `AirdropOperations|contract:${contractAddress}|amount:${amount}|recipients:${recipients}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">AirdropOperations</h3>
      <p className="text-sm text-gray-600 mb-4">Track AirdropOperations operations and distributions.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !contractAddress.startsWith('0x')} className="w-full px-4 py-3 bg-teal-600 text-white rounded-lg font-semibold hover:bg-teal-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record AirdropOperations'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-teal-700 bg-teal-50 border border-teal-200 rounded-lg p-3">✓ AirdropOperations recorded onchain.</div>}
    </section>
  )
}
