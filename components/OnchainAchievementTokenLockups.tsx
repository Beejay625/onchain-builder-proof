...
interface OnchainAchievementTokenLockupsProps {
  achievementId: bigint
}

export default function OnchainAchievementTokenLockups({ achievementId }: OnchainAchievementTokenLockupsProps) {
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">TokenLockups</h3>
      <p className="text-sm text-gray-600 mb-4">Track TokenLockups operations and reward configurations.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !poolAddress.startsWith('0x')} className="w-full px-4 py-3 bg-cyan-600 text-white rounded-lg font-semibold hover:bg-cyan-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record TokenLockups'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-cyan-700 bg-cyan-50 border border-cyan-200 rounded-lg p-3">✓ TokenLockups recorded onchain.</div>}
    </section>
  )
}
