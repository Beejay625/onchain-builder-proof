...
interface OnchainAchievementFeeDistributionProps {
  achievementId: bigint
}

export default function OnchainAchievementFeeDistribution({ achievementId }: OnchainAchievementFeeDistributionProps) {
  ...
  const record = () => {
    ...
    const payload = `FeeDistribution|contract:${contractAddress}|amount:${amount}|recipient:${recipient}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">FeeDistribution</h3>
      <p className="text-sm text-gray-600 mb-4">Track FeeDistribution operations and distributions.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !contractAddress.startsWith('0x')} className="w-full px-4 py-3 bg-pink-600 text-white rounded-lg font-semibold hover:bg-pink-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record FeeDistribution'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-pink-700 bg-pink-50 border border-pink-200 rounded-lg p-3">✓ FeeDistribution recorded onchain.</div>}
    </section>
  )
}
