...
interface OnchainAchievementSessionKeysManagementProps {
  achievementId: bigint
}

export default function OnchainAchievementSessionKeysManagement({ achievementId }: OnchainAchievementSessionKeysManagementProps) {
  ...
  const record = () => {
    ...
    const payload = `SessionKeysManagement|contract:${contractAddress}|userOp:${userOpHash}|data:${data}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">SessionKeysManagement</h3>
      <p className="text-sm text-gray-600 mb-4">Track SessionKeysManagement operations in account abstraction protocols.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !contractAddress.startsWith('0x')} className="w-full px-4 py-3 bg-green-600 text-white rounded-lg font-semibold hover:bg-green-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record SessionKeysManagement'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-green-700 bg-green-50 border border-green-200 rounded-lg p-3">✓ SessionKeysManagement recorded onchain.</div>}
    </section>
  )
}
