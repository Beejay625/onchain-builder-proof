...
interface OnchainAchievementCurvePoolProps {
  achievementId: bigint
}

export default function OnchainAchievementCurvePool({ achievementId }: OnchainAchievementCurvePoolProps) {
  const { address } = useAccount()
  const [poolAddress, setPoolAddress] = useState('0xpool')
  const [poolType, setPoolType] = useState('CurvePool')
  ...
  const record = () => {
    ...
    const payload = `CurvePool|pool:${poolAddress}|type:${poolType}|tvl:${tvl}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">CurvePool</h3>
      <p className="text-sm text-gray-600 mb-4">Track CurvePool liquidity pool operations.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !poolAddress.startsWith('0x')} className="w-full px-4 py-3 bg-teal-600 text-white rounded-lg font-semibold hover:bg-teal-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record CurvePool'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-teal-700 bg-teal-50 border border-teal-200 rounded-lg p-3">✓ CurvePool pool recorded.</div>}
    </section>
  )
}
