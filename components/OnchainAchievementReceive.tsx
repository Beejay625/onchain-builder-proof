...
interface OnchainAchievementReceiveProps {
  achievementId: bigint
}

export default function OnchainAchievementReceive({ achievementId }: OnchainAchievementReceiveProps) {
  ...
  const record = () => {
    ...
    const payload = `Receive|data:${data}|tx:${txHash}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      ...
      <p className="text-sm text-gray-600 mb-4">Track Receive function execution.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming} className="w-full px-4 py-3 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record Receive'}
      </button>
