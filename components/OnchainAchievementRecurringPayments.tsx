...
interface OnchainAchievementRecurringPaymentsProps {
  achievementId: bigint
}

export default function OnchainAchievementRecurringPayments({ achievementId }: OnchainAchievementRecurringPaymentsProps) {
  ...
  const record = () => {
    ...
    const payload = `RecurringPayments|contract:${contractAddress}|amount:${amount}|rate:${rate}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">RecurringPayments</h3>
      <p className="text-sm text-gray-600 mb-4">Track RecurringPayments operations and payment configurations.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !contractAddress.startsWith('0x')} className="w-full px-4 py-3 bg-emerald-600 text-white rounded-lg font-semibold hover:bg-emerald-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record RecurringPayments'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg p-3">✓ RecurringPayments recorded onchain.</div>}
    </section>
  )
}
