...
interface OnchainAchievementConsensusParticipationProps {
  achievementId: bigint
}

export default function OnchainAchievementConsensusParticipation({ achievementId }: OnchainAchievementConsensusParticipationProps) {
  ...
  const record = () => {
    ...
    const payload = `ConsensusParticipation|validator:${validatorAddress}|amount:${amount}|data:${data}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">ConsensusParticipation</h3>
      <p className="text-sm text-gray-600 mb-4">Track ConsensusParticipation operations in blockchain networks.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !validatorAddress.startsWith('0x')} className="w-full px-4 py-3 bg-slate-600 text-white rounded-lg font-semibold hover:bg-slate-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record ConsensusParticipation'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-slate-700 bg-slate-50 border border-slate-200 rounded-lg p-3">✓ ConsensusParticipation recorded onchain.</div>}
    </section>
  )
}
