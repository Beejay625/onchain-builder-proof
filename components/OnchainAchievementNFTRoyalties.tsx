...
interface OnchainAchievementNFTRoyaltiesProps {
  achievementId: bigint
}

export default function OnchainAchievementNFTRoyalties({ achievementId }: OnchainAchievementNFTRoyaltiesProps) {
  ...
  const record = () => {
    ...
    const payload = `NFTRoyalties|nft:${nftAddress}|tokenId:${tokenId}|data:${data}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">NFTRoyalties</h3>
      <p className="text-sm text-gray-600 mb-4">Track NFTRoyalties operations for NFT collections.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !nftAddress.startsWith('0x')} className="w-full px-4 py-3 bg-pink-600 text-white rounded-lg font-semibold hover:bg-pink-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record NFTRoyalties'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-pink-700 bg-pink-50 border border-pink-200 rounded-lg p-3">✓ NFTRoyalties recorded onchain.</div>}
    </section>
  )
}
