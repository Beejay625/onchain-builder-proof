...
interface OnchainAchievementWormholeProps {
  achievementId: bigint
}

export default function OnchainAchievementWormhole({ achievementId }: OnchainAchievementWormholeProps) {
  ...
  const record = () => {
    ...
    const payload = `Wormhole|from:${sourceChain}|to:${targetChain}|id:${messageId}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">Wormhole</h3>
      <p className="text-sm text-gray-600 mb-4">Track Wormhole cross-chain message passing.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !messageId.startsWith('0x')} className="w-full px-4 py-3 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record Wormhole'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-lg p-3">✓ Wormhole message recorded.</div>}
    </section>
  )
}
