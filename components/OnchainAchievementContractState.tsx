...
interface OnchainAchievementContractStateProps {
  achievementId: bigint
}

export default function OnchainAchievementContractState({ achievementId }: OnchainAchievementContractStateProps) {
  ...
  const record = () => {
    ...
    const payload = `ContractState|contract:${contractAddress}|network:${network}|data:${data}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">ContractState</h3>
      <p className="text-sm text-gray-600 mb-4">Track ContractState operations and contract lifecycle.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !contractAddress.startsWith('0x')} className="w-full px-4 py-3 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record ContractState'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-lg p-3">✓ ContractState recorded onchain.</div>}
    </section>
  )
}
