...
interface OnchainAchievementCodeAnalysisProps {
  achievementId: bigint
}

export default function OnchainAchievementCodeAnalysis({ achievementId }: OnchainAchievementCodeAnalysisProps) {
  ...
  const record = () => {
    ...
    const payload = `CodeAnalysis|contract:${contractAddress}|tool:${tool}|result:${result}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">CodeAnalysis</h3>
      <p className="text-sm text-gray-600 mb-4">Track CodeAnalysis operations and analysis results.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !contractAddress.startsWith('0x')} className="w-full px-4 py-3 bg-red-600 text-white rounded-lg font-semibold hover:bg-red-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record CodeAnalysis'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg p-3">✓ CodeAnalysis recorded onchain.</div>}
    </section>
  )
}
