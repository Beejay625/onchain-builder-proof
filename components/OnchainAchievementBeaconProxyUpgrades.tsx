...
interface OnchainAchievementBeaconProxyUpgradesProps {
  achievementId: bigint
}

export default function OnchainAchievementBeaconProxyUpgrades({ achievementId }: OnchainAchievementBeaconProxyUpgradesProps) {
  ...
  const record = () => {
    ...
    const payload = `BeaconProxyUpgrades|contract:${contractAddress}|impl:${implementation}|data:${data}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">BeaconProxyUpgrades</h3>
      <p className="text-sm text-gray-600 mb-4">Track BeaconProxyUpgrades operations and configurations.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !contractAddress.startsWith('0x')} className="w-full px-4 py-3 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record BeaconProxyUpgrades'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-lg p-3">✓ BeaconProxyUpgrades recorded onchain.</div>}
    </section>
  )
}
