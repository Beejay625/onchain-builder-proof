...
interface OnchainAchievementTokenMetadataProps {
  achievementId: bigint
}

export default function OnchainAchievementTokenMetadata({ achievementId }: OnchainAchievementTokenMetadataProps) {
  ...
  const record = () => {
    if (!address) return
    if (!data.trim()) return
    const payload = `TokenMetadata|data:${data}|index:${index}|timestamp:${timestamp}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">TokenMetadata</h3>
      <p className="text-sm text-gray-600 mb-4">Track TokenMetadata operations and configurations.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !data.trim()} className="w-full px-4 py-3 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record TokenMetadata'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-lg p-3">✓ TokenMetadata recorded onchain.</div>}
    </section>
  )
}
