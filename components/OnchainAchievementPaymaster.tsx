...
interface OnchainAchievementPaymasterProps {
  achievementId: bigint
}

export default function OnchainAchievementPaymaster({ achievementId }: OnchainAchievementPaymasterProps) {
  ...
  const record = () => {
    ...
    const payload = `Paymaster|addr1:${address1}|addr2:${address2}|data:${data}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">Paymaster</h3>
      <p className="text-sm text-gray-600 mb-4">Track Paymaster operations for account abstraction.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !address1.startsWith('0x')} className="w-full px-4 py-3 bg-violet-600 text-white rounded-lg font-semibold hover:bg-violet-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record Paymaster'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-violet-700 bg-violet-50 border border-violet-200 rounded-lg p-3">✓ Paymaster recorded.</div>}
    </section>
  )
}
