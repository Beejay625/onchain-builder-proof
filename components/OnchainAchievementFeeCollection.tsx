...
interface OnchainAchievementFeeCollectionProps {
  achievementId: bigint
}

export default function OnchainAchievementFeeCollection({ achievementId }: OnchainAchievementFeeCollectionProps) {
  ...
  const record = () => {
    ...
    const payload = `FeeCollection|treasury:${treasuryAddress}|amount:${amount}|recipient:${recipient}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">FeeCollection</h3>
      <p className="text-sm text-gray-600 mb-4">Track FeeCollection operations and distributions.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !treasuryAddress.startsWith('0x')} className="w-full px-4 py-3 bg-pink-600 text-white rounded-lg font-semibold hover:bg-pink-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record FeeCollection'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-pink-700 bg-pink-50 border border-pink-200 rounded-lg p-3">✓ FeeCollection recorded onchain.</div>}
    </section>
  )
}
