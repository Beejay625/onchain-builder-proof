...
interface OnchainAchievementYieldFarmingContractsProps {
  achievementId: bigint
}

export default function OnchainAchievementYieldFarmingContracts({ achievementId }: OnchainAchievementYieldFarmingContractsProps) {
  ...
  const record = () => {
    ...
    const payload = `YieldFarmingContracts|contract:${contractAddress}|amount:${amount}|duration:${duration}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">YieldFarmingContracts</h3>
      <p className="text-sm text-gray-600 mb-4">Track YieldFarmingContracts operations and configurations.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !contractAddress.startsWith('0x')} className="w-full px-4 py-3 bg-cyan-600 text-white rounded-lg font-semibold hover:bg-cyan-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record YieldFarmingContracts'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-cyan-700 bg-cyan-50 border border-cyan-200 rounded-lg p-3">✓ YieldFarmingContracts recorded onchain.</div>}
    </section>
  )
}
