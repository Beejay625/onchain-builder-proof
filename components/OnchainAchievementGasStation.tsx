...
interface OnchainAchievementGasStationProps {
  achievementId: bigint
}

export default function OnchainAchievementGasStation({ achievementId }: OnchainAchievementGasStationProps) {
  ...
  const record = () => {
    ...
    const payload = `GasStation|addr1:${address1}|addr2:${address2}|data:${data}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">GasStation</h3>
      <p className="text-sm text-gray-600 mb-4">Track GasStation operations for account abstraction.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !address1.startsWith('0x')} className="w-full px-4 py-3 bg-violet-600 text-white rounded-lg font-semibold hover:bg-violet-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record GasStation'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-violet-700 bg-violet-50 border border-violet-200 rounded-lg p-3">✓ GasStation recorded.</div>}
    </section>
  )
}
