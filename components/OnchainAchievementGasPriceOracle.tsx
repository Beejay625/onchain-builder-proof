...
interface OnchainAchievementGasPriceOracleProps {
  achievementId: bigint
}

export default function OnchainAchievementGasPriceOracle({ achievementId }: OnchainAchievementGasPriceOracleProps) {
  ...
  const record = () => {
    if (!address) return
    if (!data.trim()) return
    const payload = `GasPriceOracle|data:${data}|index:${index}|timestamp:${timestamp}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">GasPriceOracle</h3>
      <p className="text-sm text-gray-600 mb-4">Track GasPriceOracle operations and configurations.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !data.trim()} className="w-full px-4 py-3 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record GasPriceOracle'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-lg p-3">✓ GasPriceOracle recorded onchain.</div>}
    </section>
  )
}
