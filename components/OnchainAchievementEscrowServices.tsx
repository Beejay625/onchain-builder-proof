...
interface OnchainAchievementEscrowServicesProps {
  achievementId: bigint
}

export default function OnchainAchievementEscrowServices({ achievementId }: OnchainAchievementEscrowServicesProps) {
  ...
  const record = () => {
    ...
    const payload = `EscrowServices|marketplace:${marketplaceAddress}|nft:${nftAddress}|price:${price}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">EscrowServices</h3>
      <p className="text-sm text-gray-600 mb-4">Track EscrowServices operations and marketplace interactions.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !marketplaceAddress.startsWith('0x')} className="w-full px-4 py-3 bg-rose-600 text-white rounded-lg font-semibold hover:bg-rose-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record EscrowServices'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-rose-700 bg-rose-50 border border-rose-200 rounded-lg p-3">✓ EscrowServices recorded onchain.</div>}
    </section>
  )
}
