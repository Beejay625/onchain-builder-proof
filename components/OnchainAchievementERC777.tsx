...
interface OnchainAchievementERC777Props {
  achievementId: bigint
}

export default function OnchainAchievementERC777({ achievementId }: OnchainAchievementERC777Props) {
  const { address } = useAccount()
  const [contractAddress, setContractAddress] = useState('0xcontract')
  const [standard, setStandard] = useState('ERC777')
  ...
  const record = () => {
    ...
    const payload = `ERC777|contract:${contractAddress}|standard:${standard}|impl:${implementation}`
    writeContract({
  ...
  return (
    <section className="bg-white border rounded-xl shadow p-6">
      <h3 className="text-xl font-bold mb-2">ERC777</h3>
      <p className="text-sm text-gray-600 mb-4">Track ERC777 token standard implementations.</p>
      ...
      <button onClick={record} disabled={isPending || isConfirming || !address || !contractAddress.startsWith('0x')} className="w-full px-4 py-3 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 disabled:bg-gray-400">
        {isPending || isConfirming ? 'Recording...' : 'Record ERC777'}
      </button>
      {isSuccess && <div className="mt-4 text-sm text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-lg p-3">✓ ERC777 implementation recorded.</div>}
    </section>
  )
}
